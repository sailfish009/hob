...
(defun is-arrow (word)
  (and (> (length word) 1)
       (eql (schar word (1- (length word))) #\>)
       (find (schar word 0) *operator-chars*)))
          ...
          ((is-word-char ch)
           (let ((word (read-word in ch)) num)
             ...
             (cond (num (values :num num))
                   ...
                   ((eql (cur-ch in) #\{) (next in) (values :punc (concatenate 'string word "{")))
                   ((or (is-arrow word) (equal word ":")) (values :punc word))
...
(defun read-word (in start)
  (with-output-to-string (out)
    ...
    (loop :with prev := start :for cur := (cur-ch in)
          :while (and cur (is-word-char cur)
                      (or (not (eql cur #\:)) (not prev) (find prev *operator-chars*))) :do
       (write-char (setf prev cur) out)
       (next in))))
